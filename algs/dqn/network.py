...
print('network.py using device:', device)
...
class Model_2x2(nn.Module):
    def __init__(self, env):
        ...
        self.conv1 = torch.nn.Conv2d(in_channels, 8, kernel_size=1, stride=1)
...
class Model_10x10(nn.Module):
    def __init__(self, env):
        ...
        in_channels = env.observation_space.shape[0]
        out_features = env.action_space.n
        self.conv1 = torch.nn.Conv2d(in_channels, 32, kernel_size=1, stride=1)
    ...
    def forward(self, orig_x):
        orig_x = torch.from_numpy(orig_x).float().to(device)
