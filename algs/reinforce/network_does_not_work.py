...
class Model_2x2(nn.Module):
    def __init__(self, env):
        ...
        self.base_model = ReinforceBaseModel(env, 16)
        self.conv1 = torch.nn.Conv2d(self.base_model.n_channels, 8, kernel_size=1, stride=1)
        self.conv2 = torch.nn.Conv2d(8, 16, kernel_size=1, stride=1)
        self.conv3 = torch.nn.Conv2d(16, 16, kernel_size=2, stride=1)
        self.affine1 = nn.Linear(1 * 1 * 16, self.base_model.hidden_out)
...
class Model_10x10(ReinforceBaseModel):
    def __init__(self, env):
        super(Model_10x10, self).__init__(env, 512)

        self.conv1 = torch.nn.Conv2d(self.n_channels, 32, kernel_size=1, stride=1)
        ...
        self.affine1 = nn.Linear(128 * 2 * 2, self.hidden_out)
    ...
    def forward(self, orig_x):
        ...
        return super(Model_10x10, self).forward(x)
