...
class ModelAlice_2x2(nn.Module):
    CONTINUE = 0
    STOP = 1
    def __init__(self, env):
        super(ModelAlice_2x2, self).__init__()
...
class ModelAlice_10x10(nn.Module):
    CONTINUE = 0
    STOP = 1
    def __init__(self, env):
        super(ModelAlice_10x10, self).__init__()
        self.n_inputs = env.observation_space.shape[0]
        self.n_outputs = 2
        ...
        self.affine1 = nn.Linear(128 * 2 * 2 + 3, 512)
        # actor's layer
        self.action_head = nn.Linear(self.affine1.out_features, self.n_outputs)
    ...
    def forward(self, orig_x, orig_x2):
        x = F.relu(self.conv1(orig_x))
        x = F.relu(self.conv2(x))
        x = F.relu(self.conv3(x))
        x = F.relu(self.conv4(x))
        x = torch.flatten(x, start_dim=1)
        x = torch.cat((x, orig_x2), dim=1)
        x = F.relu(self.affine1(x))
...
def get_model_class(params, alice: bool = False):
    if params.size == '2x2':
        return ModelAlice_2x2 if alice else Model_2x2
    if params.size == '10x10':
        return ModelAlice_10x10 if alice else Model_10x10
